...
class RuleInlinerImpl : public InPlaceGrammarRewriter {
 protected:
  void Prepare() override {
    ...
    for (int32_t rule_id = 0; rule_id < num_rules; ++rule_id) {
      const auto body = builder_.GetGrammarExpr(original_body_expr_ids_[rule_id]);
 ...
 private:
  ...
  bool CanRuleBeInlined(int32_t rule_id) {
    ...
    if (rule.max_tokens >= 0 || rule.max_chars >= 0 || !rule.capture_name.empty() ||
        ...
        rule.temperature.has_value() || is_unique_key_scope_rule_[rule_id]) {
...
class GrammarFSMBuilderImpl {
 ...
 private:
  ...
  static Result<FSMWithStartEnd> RepeatRegularFSM(
      const FSMWithStartEnd& child, int32_t lower, int32_t upper
  );
...
Result<FSMWithStartEnd> GrammarFSMBuilderImpl::RepeatRegularFSM(
  ...
  if (copies * (static_cast<int64_t>(child.NumStates()) + 2) > kMaxDynamicTagNameStates) {
    return ResultErr(
        "DynamicTag name repetition expands beyond 100000 FSM states; use a regex name rule "
        "or a smaller bound"
    );
  }
    ...
    for (const auto& edge : raw_fsm.GetFsm().GetEdges(source)) {
      if (edge.IsCharRange()) {
        ...
        auto repeated_result =
            RepeatRegularFSM(std::move(child_result).Unwrap(), repeat.Lower(), repeat.Upper());
