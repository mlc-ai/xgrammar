...
std::string FormatToJSONStr(const Format& format) {
  return std::visit([&](auto&& arg) -> std::string { return arg.ToJSON(); }, format);
}
...
Result<std::vector<Layers>, StructuralTagError> StructuralTagTemplateFiller::Visit(
  ...
  auto serialized_format =
      std::visit([&](auto&& arg) -> std::string { return arg.ToJSON(); }, format);
...
Result<std::vector<Format>, StructuralTagError> StructuralTagTemplateFiller::VisitExpandSub(
  ...
  for (int i = static_cast<int>(current_expanded_placeholder_and_index_.size());
    ...
    XGRAMMAR_DCHECK(current_value.get().contains(ph.name))
        ...
        << FormatToJSONStr(format_template_to_expand) << "'" << "\n"
...
Result<std::vector<Format>, StructuralTagError> StructuralTagTemplateFiller::VisitExpandSub(
  ...
  if (!need_expand) {
    ...
    auto end_str = ReplacePlaceHolder(format_template_to_expand.end);
    if (end_str.IsErr()) {
      return ResultErr(std::move(end_str).UnwrapErr());
    }
    auto unwrapped_end_str = std::move(end_str).Unwrap();
...
std::string SequenceFormat::ToJSON() const {
  std::string repr = "{\"type\": \"sequence\", \"elements\": [";
  for (size_t i = 0; i < elements.size(); ++i) {
    repr += std::visit([&](auto&& arg) -> std::string { return arg.ToJSON(); }, elements[i]);
...
std::string OrFormat::ToJSON() const {
...
std::string TagsWithSeparatorFormat::ToJSON() const {
