...
template <typename T>
std::uintptr_t CheckTensorDataAddress(const DLTensor* tensor, const char* name) {
  XGRAMMAR_CHECK(tensor->data != nullptr) << "The " << name << " tensor data must not be null";
  const auto base_address = reinterpret_cast<std::uintptr_t>(tensor->data);
  XGRAMMAR_CHECK(tensor->byte_offset <= std::numeric_limits<std::uintptr_t>::max() - base_address)
      << "The " << name << " tensor byte_offset overflows the address space";
  const auto data_address = base_address + static_cast<std::uintptr_t>(tensor->byte_offset);
  XGRAMMAR_CHECK(data_address % alignof(T) == 0)
      << "The " << name << " tensor data pointer plus byte_offset must be aligned to " << alignof(T)
      << " bytes";
  return data_address;
}

template <typename T>
const T* CheckAndGetTensorData(const DLTensor* tensor, const char* name) {
  return reinterpret_cast<const T*>(CheckTensorDataAddress<T>(tensor, name));
}

template <typename T>
T* CheckAndGetTensorData(DLTensor* tensor, const char* name) {
  return reinterpret_cast<T*>(CheckTensorDataAddress<T>(tensor, name));
}
...
bool TraverseDraftTreeRecursive(
  ...
  bool accepted;
  if (current_position == root_position) {
...
int32_t* CheckAndGetBitmaskPtr(const DLTensor& token_bitmask, int vocab_size, int index) {
  ...
  return const_cast<int32_t*>(
             details::CheckAndGetTensorData<int32_t>(&token_bitmask, "token_bitmask")
         ) +
         index * buffer_size;
}
...
class BatchGrammarMatcher::Impl {
 public:
  Impl(std::variant<std::string, int32_t> max_threads) {
    const int32_t hardware_threads =
      ...
      if (num_threads > 1) {
        if (num_threads > hardware_threads) {
          XGRAMMAR_LOG(WARNING) << "The num_threads " << num_threads << " is larger than the "
                                << "number of hardware threads. Using " << hardware_threads
                                << " instead.";
        }
        max_threads_ = std::min(num_threads, hardware_threads);
      }
    } else {
      std::string str = std::get<std::string>(max_threads);
      XGRAMMAR_CHECK(str == "auto");
      max_threads_ = std::max(int32_t{1}, hardware_threads / 2);
    }
    if (max_threads_ > 1) {
  ...
  std::vector<uint8_t> BatchTraverseDraftTree(
      ...
      DLTensor* token_bitmask,
      const std::optional<std::vector<int32_t>>& indices,
      const std::optional<std::vector<int32_t>>& root_positions,
      double time_threshold
  );
...
void BatchGrammarMatcher::Impl::BatchFillNextTokenBitmask(
  ...
  for (int32_t i = 0; i < static_cast<int32_t>(matchers->size()); ++i) {
    const int32_t index = indices.has_value() ? (*indices)[i] : i;
  ...
  auto fill_next_token_mask = [&](int32_t batch_id) {
    ...
    const int32_t index = indices.has_value() ? (*indices)[batch_id] : batch_id;
  ...
  const int32_t batch_size = static_cast<int32_t>(matchers->size());
  if (batch_size == 0) {
    return;
  }
  const int32_t num_tasks = std::min(max_threads_, batch_size);
  const int32_t matchers_per_task = (batch_size + num_tasks - 1) / num_tasks;
  for (int32_t task = 0; task < num_tasks; ++task) {
    const int32_t begin = task * matchers_per_task;
    const int32_t end = std::min(begin + matchers_per_task, batch_size);
    thread_pool_->Execute([&fill_next_token_mask, begin, end]() {
...
std::vector<uint8_t> BatchGrammarMatcher::Impl::BatchTraverseDraftTree(
    std::vector<GrammarMatcher>* matchers,
    const DLTensor* retrieve_next_token,
    const DLTensor* retrieve_next_sibling,
    const DLTensor* draft_tokens,
  ...
  XGRAMMAR_CHECK(!indices.has_value() || indices->size() == matchers->size())
      << "The size of indices (" << (indices.has_value() ? indices->size() : 0)
      << ") should be the same as the size of matchers (" << matchers->size() << ").";
  XGRAMMAR_CHECK(!root_positions.has_value() || root_positions->size() == matchers->size())
      << "The size of root_positions (" << (root_positions.has_value() ? root_positions->size() : 0)
      << ") should be the same as the size of matchers (" << matchers->size() << ").";

  auto check_cpu_int64 = [](const DLTensor* tensor, const char* name) {
    XGRAMMAR_CHECK(
    ...
    XGRAMMAR_CHECK(tensor->dtype.code == kDLInt && tensor->dtype.bits == 64)
        << "The " << name << " tensor must have int64 dtype";
  };
  auto is_contiguous = [](const DLTensor* tensor) {
    if (tensor->strides == nullptr) {
      return true;
    }
    int64_t expected_stride = 1;
    for (int dim = tensor->ndim - 1; dim >= 0; --dim) {
  ...
  check_cpu_int64(retrieve_next_token, "retrieve_next_token");
  check_cpu_int64(retrieve_next_sibling, "retrieve_next_sibling");
  check_cpu_int64(draft_tokens, "draft_tokens");
  XGRAMMAR_CHECK(retrieve_next_token->ndim == 1 || retrieve_next_token->ndim == 2)
      << "The retrieve_next_token tensor must be 1D or 2D";
  XGRAMMAR_CHECK(retrieve_next_sibling->ndim == retrieve_next_token->ndim)
      << "The retrieve_next_sibling tensor rank must match retrieve_next_token";
  XGRAMMAR_CHECK(draft_tokens->ndim == 2) << "The draft_tokens tensor must be 2D";
  XGRAMMAR_CHECK(
  ...
  XGRAMMAR_CHECK(
      ...
      token_bitmask->device.device_type == kDLCUDAHost ||
      token_bitmask->device.device_type == kDLROCMHost
  ) << "The token_bitmask tensor must be on CPU";
  XGRAMMAR_CHECK(
      is_contiguous(retrieve_next_token) && is_contiguous(retrieve_next_sibling) &&
      is_contiguous(draft_tokens) && is_contiguous(token_bitmask)
  ) << "BatchTraverseDraftTree requires contiguous tensors";

  const int64_t batch_size = draft_tokens->shape[0];
  const int64_t num_nodes = draft_tokens->shape[1];
  XGRAMMAR_CHECK(batch_size >= 0) << "The draft_tokens batch dimension must not be negative";
  XGRAMMAR_CHECK(num_nodes > 0) << "The draft tree must not be empty";
  const int tree_node_dim = retrieve_next_token->ndim - 1;
  ...
  if (retrieve_next_token->ndim == 2) {
    XGRAMMAR_CHECK(
        retrieve_next_token->shape[0] == batch_size && retrieve_next_sibling->shape[0] == batch_size
    ) << "The batched tree tensors must match the draft_tokens batch size";
  }
  XGRAMMAR_CHECK(token_bitmask->shape[0] == batch_size * num_nodes)
      << "The token_bitmask row count must equal batch_size * num_nodes";

  auto row_index = [&](int32_t matcher_index) {
    const int32_t index = indices.has_value() ? (*indices)[matcher_index] : matcher_index;
    XGRAMMAR_CHECK(index >= 0 && index < batch_size)
        << "The index " << index << " is out of range [0, " << batch_size << ") for matcher "
        << matcher_index;
    return index;
  };
  std::vector<int32_t> matcher_by_row(static_cast<std::size_t>(batch_size), -1);
  for (int32_t i = 0; i < static_cast<int32_t>(matchers->size()); ++i) {
    const int32_t index = row_index(i);
    XGRAMMAR_CHECK(matcher_by_row[index] == -1)
        << "The index " << index << " is assigned to more than one matcher";
    matcher_by_row[index] = i;
    const int32_t root_position = root_positions.has_value() ? (*root_positions)[i] : 0;
    XGRAMMAR_CHECK(root_position >= 0 && root_position < num_nodes)
        << "The root position " << root_position << " is out of range [0, " << num_nodes
        << ") for matcher " << i;
  }
  ...
  const auto* next_token_data =
      details::CheckAndGetTensorData<int64_t>(retrieve_next_token, "retrieve_next_token");
  const auto* next_sibling_data =
      details::CheckAndGetTensorData<int64_t>(retrieve_next_sibling, "retrieve_next_sibling");
  const auto* draft_token_data =
      details::CheckAndGetTensorData<int64_t>(draft_tokens, "draft_tokens");
  auto* mask_data = details::CheckAndGetTensorData<int32_t>(token_bitmask, "token_bitmask");
  const int64_t mask_words_per_request = num_nodes * token_bitmask->shape[1];

  std::vector<uint8_t> completed(matchers->size(), 0);
  auto traverse = [&](int32_t matcher_index) {
    const int32_t batch_index = row_index(matcher_index);
    const int32_t tree_index = retrieve_next_token->ndim == 1 ? 0 : batch_index;
    const int32_t root_position = root_positions.has_value() ? (*root_positions)[matcher_index] : 0;
    const int64_t mask_size = token_bitmask->shape[1];
    int64_t tree_shape[1] = {num_nodes};
    int64_t mask_shape[2] = {num_nodes, mask_size};

    auto make_int64_row = [&](const DLTensor* source, const int64_t* source_data, int32_t source_row
                          ) {
      DLTensor view = *source;
      view.data = const_cast<int64_t*>(source_data + static_cast<int64_t>(source_row) * num_nodes);
      view.byte_offset = 0;
      view.ndim = 1;
      view.shape = tree_shape;
      view.strides = nullptr;
      return view;
    };

    DLTensor next_token_row = make_int64_row(retrieve_next_token, next_token_data, tree_index);
    DLTensor next_sibling_row =
        make_int64_row(retrieve_next_sibling, next_sibling_data, tree_index);
    DLTensor draft_token_row = make_int64_row(draft_tokens, draft_token_data, batch_index);
    DLTensor bitmask_view = *token_bitmask;
    bitmask_view.data = mask_data + static_cast<int64_t>(batch_index) * num_nodes * mask_size;
    bitmask_view.byte_offset = 0;
    bitmask_view.ndim = 2;
    bitmask_view.shape = mask_shape;
    bitmask_view.strides = nullptr;

    completed[matcher_index] = static_cast<uint8_t>((*matchers)[matcher_index].TraverseDraftTree(
        &next_token_row,
        &next_sibling_row,
        &draft_token_row,
        &bitmask_view,
  ...
  auto process_rows = [&](int64_t begin, int64_t end) {
    for (int64_t batch_index = begin; batch_index < end; ++batch_index) {
      std::fill_n(
          mask_data + batch_index * mask_words_per_request, mask_words_per_request, int32_t{-1}
      );
      const int32_t matcher_index = matcher_by_row[batch_index];
      if (matcher_index >= 0) {
        traverse(matcher_index);
      }
    }
  };

  if (!thread_pool_.has_value()) {
    process_rows(0, batch_size);
  } else {
    const int32_t num_tasks = std::min<int32_t>(max_threads_, batch_size);
    const int64_t rows_per_task = (batch_size + num_tasks - 1) / num_tasks;
    for (int32_t task = 0; task < num_tasks; ++task) {
      const int64_t begin = task * rows_per_task;
      const int64_t end = std::min<int64_t>(begin + rows_per_task, batch_size);
      thread_pool_->Execute([&process_rows, begin, end]() { process_rows(begin, end); });
    }
    thread_pool_->Wait();
  }
  return completed;
...
bool GrammarMatcher::TraverseDraftTree(
  ...
  XGRAMMAR_CHECK(root_position >= 0 && root_position < retrieve_next_sibling->shape[0])
      << "The root position must be in the draft tree";
  const auto* next_token_data =
      details::CheckAndGetTensorData<int64_t>(retrieve_next_token, "retrieve_next_token");
  const auto* next_sibling_data =
      details::CheckAndGetTensorData<int64_t>(retrieve_next_sibling, "retrieve_next_sibling");
  const auto* draft_token_data =
      details::CheckAndGetTensorData<int64_t>(draft_tokens, "draft_tokens");
  auto* bitmask_data = details::CheckAndGetTensorData<int32_t>(token_bitmask, "token_bitmask");
  float* temperatures_data = nullptr;
  if (temperatures != nullptr) {
    temperatures_data = details::CheckAndGetTensorData<float>(temperatures, "temperatures");
    std::fill_n(temperatures_data, temperatures->shape[0], -1.0f);
  }
  const int32_t bitmask_size = static_cast<int32_t>(token_bitmask->shape[1]);
  XGRAMMAR_CHECK(next_sibling_data[root_position] == -1) << "The root node must not have siblings";
...
void BatchGrammarMatcher::BatchFillTemperature(
  ...
  auto* temperatures_data = details::CheckAndGetTensorData<float>(temperatures, "temperatures");
...
std::vector<uint8_t> BatchGrammarMatcher::BatchTraverseDraftTree(
    std::vector<GrammarMatcher>* matchers,
    const DLTensor* retrieve_next_token,
    const DLTensor* retrieve_next_sibling,
    const DLTensor* draft_tokens,
  ...
  return pimpl_->BatchTraverseDraftTree(
      matchers,
      retrieve_next_token,
      retrieve_next_sibling,
      draft_tokens,
