...
bool TraverseDraftTreeRecursive(
  ...
  int32_t* bitmask_data = reinterpret_cast<int32_t*>(
      static_cast<char*>(token_bitmask->data) + token_bitmask->byte_offset
  );
...
std::vector<uint8_t> BatchGrammarMatcher::Impl::BatchTraverseDraftTree(
  ...
  auto* mask_data = reinterpret_cast<int32_t*>(
      static_cast<char*>(token_bitmask->data) + token_bitmask->byte_offset
  );
  ...
  auto traverse = [&](int32_t matcher_index) {
    ...
    auto make_int64_row = [&](const DLTensor* source, int32_t source_row) {
      DLTensor view = *source;
      auto* base = static_cast<char*>(source->data) + source->byte_offset;
      view.data = base + static_cast<int64_t>(source_row) * num_nodes * sizeof(int64_t);
      view.byte_offset = 0;
      view.ndim = 1;
    ...
    DLTensor next_token_row = make_int64_row(retrieve_next_token, tree_index);
    DLTensor next_sibling_row = make_int64_row(retrieve_next_sibling, tree_index);
    DLTensor draft_token_row = make_int64_row(draft_tokens, batch_index);
    DLTensor bitmask_view = *token_bitmask;
    auto* mask_base = static_cast<char*>(token_bitmask->data) + token_bitmask->byte_offset;
    bitmask_view.data =
        mask_base + static_cast<int64_t>(batch_index) * num_nodes * mask_size * sizeof(int32_t);
  ...
  if (!thread_pool_.has_value() || batch_size == 0) {
...
bool GrammarMatcher::TraverseDraftTree(
  ...
  const auto* draft_token_data = reinterpret_cast<const int64_t*>(
      static_cast<const char*>(draft_tokens->data) + draft_tokens->byte_offset
  );
