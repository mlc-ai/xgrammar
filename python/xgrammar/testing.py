...
def _json_schema_to_ebnf(
    ...
    *,
    ...
    json_format : str, default: "json"
        ...
        "minimax_m3_xml", "deepseek_xml", "glm_xml", "cohere_xml", or "kimi_k3_xml". MiniMax M3
        and Cohere encode nested objects and arrays recursively; the other XML styles use JSON for
        nested values. MiniMax M3 grammars contain a serializable ``DynamicTag`` expression, so
        matching opening and closing names are enforced when the returned EBNF is compiled
        directly as well as through a structural tag.
