...
template <typename T>
void apply_token_bitmask_cpu_impl(
...
void ApplyXGrammarTokenBitmask::eval_cpu(
  ...
  if (out.dtype() == mx::float32) {
    return apply_token_bitmask_cpu_impl<float>(bitmask, logits, out, stream());
...
std::vector<mx::array> ApplyXGrammarTokenBitmask::jvp(
  ...
  if (argnums.size() == 1 && argnums[0] == 1) {
    return {apply_token_bitmask(primals[0], tangents[0], stream())};
  }
...
std::vector<mx::array> ApplyXGrammarTokenBitmask::
  ...
  // Only propagate gradients through logits, not bitmask
  if (argnums.size() == 1 && argnums[0] == 1) {
    return {apply_token_bitmask(primals[0], cotangents[0], stream())};
  }
