...
Not every JSON Schema keyword is expressible as a context-free grammar, so some
are not enforced by the compiled grammar. Where a keyword is not enforced, the
grammar is *more permissive* than the schema: generation can produce output that
the schema itself would reject.

The table below is generated by
[`tests/python/test_json_schema_keyword_support.py`](https://github.com/mlc-ai/xgrammar/blob/main/tests/python/test_json_schema_keyword_support.py),
which compares each keyword against the `jsonschema` reference implementation,
so it stays in step with the code.

| keyword | status | note |
|---|---|---|
| `type` (all seven) | enforced | |
| `enum`, `const` | enforced | |
| `minLength`, `maxLength`, `pattern` | enforced | |
| `format` | enforced | |
| `minimum`, `maximum` | enforced | integer and number |
| `exclusiveMinimum`, `exclusiveMaximum` | enforced | |
| `minItems`, `maxItems` | enforced | |
| `required` | enforced | |
| `additionalProperties: false` | enforced | |
| `anyOf`, `oneOf` | enforced | |
| `multipleOf` | enforced for `integer` only | ignored for `number`; warns at compile time |
| `allOf` | not enforced | warns at compile time |
| `uniqueItems` | not enforced | no warning ([#932](https://github.com/mlc-ai/xgrammar/issues/932)) |
| `not` | not enforced | no warning |

`additionalProperties` behaves differently from the specification by design.
With `strict_mode=True` (the default) the grammar rejects properties that are
not declared in the schema, whereas the specification permits them when
`additionalProperties` is omitted. Pass `strict_mode=False` for specification
behaviour:

```python
schema = {"type": "object", "properties": {"a": {"type": "integer"}}}

# default: additional properties are rejected
xgr.Grammar.from_json_schema(json.dumps(schema))

# specification behaviour: additional properties are allowed
xgr.Grammar.from_json_schema(json.dumps(schema), strict_mode=False)
```

If your schema relies on a keyword that is not enforced, validate the generated
output against the schema after generation as well.
