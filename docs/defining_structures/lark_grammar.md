...
`temperature` can be combined with `capture`. Combining it with `max_tokens`, `max_chars`, `lazy`,
`suffix`, or `stop` is not supported.
...
The matcher forces the occurrence to end at the character boundary when its body can end there. If
the body cannot end, the budget is relaxed until the earliest valid end; a warning is logged the
first time this consumes too many characters, at most once per matcher. Prefer bodies such as
arbitrary text that can end after every codepoint when an exact bound is required.

The budget applies per occurrence and nested budgets take the minimum. `max_chars` can be combined
with `max_tokens`, `lazy`, `suffix`, and `stop`; the first applicable boundary wins. It also applies
to `accept_string`, unlike `max_tokens`. Rollback, reset, forking, and speculative decoding restore
the character count exactly.

`max_chars` must be between 1 and 1,000,000, inclusive. It cannot be used on terminals, with
`temperature`, or on rules consumed by the dynamic dispatch pattern.
