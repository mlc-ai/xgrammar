...
Enforcement follows the same earliest-valid-boundary rule as `max_tokens`. A body such as `[^]*`,
which can end after every codepoint, is bounded exactly. If the body cannot end when the character
budget is exhausted, the matcher relaxes the budget until the earliest valid end and logs a
warning the first time, at most once per matcher.
