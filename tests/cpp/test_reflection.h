...
// An example of member equality trait
namespace xgrammar {

template <bool SkipDefault = false, typename T>
inline bool AutoCompareEq(const T& lhs, const T& rhs) {
  ...
  if constexpr (!SkipDefault && details::has_equality<T>::value) {
    ...
    return std::apply(
        [&lhs, &rhs](auto&&... member_ptr) {
          return (AutoCompareEq(lhs.*member_ptr, rhs.*member_ptr) && ...);
        },
        Functor::members
    );
  } else if constexpr (Functor::value == member_type::kDelegate) {
    return AutoCompareEq(Functor::into(lhs), Functor::into(rhs));
  } else {
