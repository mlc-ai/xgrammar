...
# Note: With deduplication optimization, f1 and f2 (both using Schema1) share the same grammar rules.
# This significantly reduces the total number of rules (only 2 sets instead of 3).
...
triggered_tags_group ::= (("1>" root "</function>") | ("2>" root "</function>"))
...
# Note: With deduplication optimization, f1 and f2 (both using Schema1) share the same grammar rules.
# This reduces the total number of rules from 3 complete sets to 2 sets (one for Schema1, one for Schema2).
