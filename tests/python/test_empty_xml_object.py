...
XML_FORMATS = ["qwen_xml", "minimax_xml", "deepseek_xml", "glm_xml", "kimi_k3_xml"]
...
@pytest.mark.parametrize(
    ...
    ids=["annotations", "min_properties", "max_properties", "unevaluated"],
)
...
def test_max_properties_zero_closes_open_object():
    grammar = _json_schema_to_ebnf(
    ...
    assert _is_grammar_accept_string(grammar, "")
    assert not _is_grammar_accept_string(grammar, "\t")
    assert not _is_grammar_accept_string(grammar, "<parameter=city>Paris</parameter>")


def test_max_properties_zero_closes_object_with_declared_property():
    grammar = _json_schema_to_ebnf(
        {
            "type": "object",
