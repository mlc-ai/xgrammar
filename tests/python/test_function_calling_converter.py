...
def test_minimax_m3_property_names_preserves_the_dynamic_value_policy():
    schema = {
        "type": "object",
        "propertyNames": {"pattern": "^[a-z]+$"},
        "additionalProperties": {"type": "string"},
...
def test_minimax_m3_dynamic_property_names_are_unique_per_object():
    grammar = _json_schema_to_ebnf(
        {"type": "object", "additionalProperties": {"type": "string"}}, json_format="minimax_m3_xml"
    )
    assert _is_grammar_accept_string(grammar, _m3_element("left", "a") + _m3_element("right", "b"))
    assert not _is_grammar_accept_string(
        grammar, _m3_element("same", "a") + _m3_element("same", "b")
    )


def test_minimax_m3_dynamic_property_uniqueness_is_nested_object_local():
    schema = {
        "type": "object",
...
def test_minimax_m3_property_names_cannot_repeat_a_fixed_property():
    schema = {
        "type": "object",
    ...
    assert _is_grammar_accept_string(grammar, fixed + _m3_element("other", "value"))
    assert not _is_grammar_accept_string(grammar, fixed + _m3_element("fixed", "again"))


@pytest.mark.parametrize("fixed_name", ["fixed", "城市"])
@pytest.mark.parametrize("with_pattern_properties", [False, True])
def test_minimax_m3_additional_properties_cannot_repeat_fixed_property(
    fixed_name: str, with_pattern_properties: bool
):
    schema = {
        "type": "object",
        "properties": {fixed_name: {"type": "string"}},
        "required": [fixed_name],
        "additionalProperties": {"type": "integer"},
    }
    if with_pattern_properties:
        schema["patternProperties"] = {"^.*$": {"type": "integer"}}
    grammar = _json_schema_to_ebnf(schema, json_format="minimax_m3_xml")
    fixed = _m3_element(fixed_name, "value")

    assert _is_grammar_accept_string(grammar, fixed)
    assert _is_grammar_accept_string(grammar, fixed + _m3_element("other", "1"))
    assert _is_grammar_accept_string(grammar, fixed + _m3_element(fixed_name[:-1], "1"))
    assert _is_grammar_accept_string(grammar, fixed + _m3_element(fixed_name + "_extra", "1"))
    assert not _is_grammar_accept_string(grammar, fixed + _m3_element(fixed_name, "1"))
