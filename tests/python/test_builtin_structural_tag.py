...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "type": "object",
                "propertyNames": {"minLength": 2, "maxLength": 4},
                "additionalProperties": {"type": "string"},
            },
            _m3_element("name", "value"),
            _m3_element("x", "value"),
        ),
        (
            {
                "type": "object",
                "propertyNames": {"pattern": "^[a-z]{1,200}$"},
                "additionalProperties": {"type": "string"},
            },
            _m3_element("a" * 200, "value"),
            _m3_element("a" * 201, "value"),
        ),
        (
            {
                "type": "object",
...
def test_minimax_m3_runtime_schema_uses_dynamic_tag(schema: Dict[str, Any]):
    structural_tag = get_model_structural_tag(
        "minimax_m3",
    ...
    grammar = xgr.Grammar.from_structural_tag(structural_tag)
    assert "DynamicTag(" in str(grammar)
...
def test_minimax_m3_duplicate_dynamic_key_is_filtered_at_opening_suffix():
    structural_tag = get_model_structural_tag(
        "minimax_m3",
        tools=make_tools(
    ...
    tokenizer_info = xgr.TokenizerInfo(["same>", "other>"])
    compiled = xgr.GrammarCompiler(tokenizer_info).compile_structural_tag(structural_tag)
    compiled = xgr.CompiledGrammar.deserialize_json(compiled.serialize_json(), tokenizer_info)
    matcher = xgr.GrammarMatcher(compiled, terminate_without_stop_token=True)
    assert matcher.accept_string(
        f"{_M3_NS}<tool_call>\n"
        f'{_M3_NS}<invoke name="dynamic">'
        f"{_M3_NS}<same>first{_M3_NS}</same>"
        f"{_M3_NS}<"
    )

    bitmask = xgr.allocate_token_bitmask(1, tokenizer_info.vocab_size)
    matcher.fill_next_token_bitmask(bitmask)
    assert _get_masked_tokens_from_bitmask(bitmask, tokenizer_info.vocab_size) == [0]
    assert not matcher.accept_token(0)
...
def test_minimax_m3_dynamic_rollback_interleaves_compact_and_full_checkpoints():
    structural_tag = get_model_structural_tag(
        "minimax_m3",
        tools=make_tools(
            ["dynamic"], {"type": "object", "additionalProperties": {"type": "string"}}
    ...
    name_chunk = "runtime_key_segment_"
    old_name = name_chunk * 2 + "old"
    new_name = name_chunk * 2 + "new"
    tokenizer_info = xgr.TokenizerInfo([name_chunk, "old", "new", ">"])
    compiled = xgr.GrammarCompiler(tokenizer_info).compile_structural_tag(structural_tag)
    matcher = xgr.GrammarMatcher(compiled, terminate_without_stop_token=True)

    assert matcher.accept_string(f'{_M3_NS}<tool_call>\n{_M3_NS}<invoke name="dynamic">{_M3_NS}<')
    assert matcher.accept_token(0)
    assert matcher.accept_token(0)
    assert matcher.accept_token(1)
    assert matcher.accept_token(3)
    ...
    # Roll back the full opening-tag transition and then the preceding compact name append.
    matcher.rollback(2)
    assert matcher.accept_token(2)
    assert matcher.accept_token(3)
    assert matcher.accept_string(
        f"value{_M3_NS}</{new_name}>{_M3_NS}</invoke>\n{_M3_NS}</tool_call>"
    )
    assert matcher.is_completed()

    assert forked.accept_string(
        f"value{_M3_NS}</{old_name}>{_M3_NS}</invoke>\n{_M3_NS}</tool_call>"
    )
...
def test_minimax_m3_dynamic_tag_survives_ebnf_and_compiler_cache(dynamic_first: bool):
    structural_tag = get_model_structural_tag(
        "minimax_m3",
        tools=make_tools(
            ["dynamic"], {"type": "object", "additionalProperties": {"type": "string"}}
    ...
    reparsed_grammar = xgr.Grammar.from_ebnf(str(dynamic_grammar))
    valid = _m3_tool_block(
        _m3_invoke_with_body("dynamic", f"{_M3_NS}<runtime_key>value{_M3_NS}</runtime_key>")
    )
    invalid = _m3_tool_block(
        _m3_invoke_with_body("dynamic", f"{_M3_NS}<runtime_key>value{_M3_NS}</wrong_key>")
    )
    duplicate = _m3_tool_block(
        _m3_invoke_with_body(
            "dynamic",
            f"{_M3_NS}<runtime_key>first{_M3_NS}</runtime_key>"
            f"{_M3_NS}<runtime_key>second{_M3_NS}</runtime_key>",
        )
    )

    compiler = xgr.GrammarCompiler(xgr.TokenizerInfo(["x"]), cache_enabled=True)
    ordered = (
        [("dynamic", dynamic_grammar), ("reparsed", reparsed_grammar)]
        if dynamic_first
        else [("reparsed", reparsed_grammar), ("dynamic", dynamic_grammar)]
    )
    compiled = {name: compiler.compile_grammar(grammar) for name, grammar in ordered}

    for grammar in compiled.values():
        assert _is_grammar_accept_string(grammar.grammar, valid)
        assert not _is_grammar_accept_string(grammar.grammar, invalid)
        assert not _is_grammar_accept_string(grammar.grammar, duplicate)
